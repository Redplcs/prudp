//  
//   (c) Copyright 1998-2000, Proksim Inc.
//   ALL RIGHTS RESERVED
//  
//  

#ifndef _LZssPlugin_H_
#define _LZssPlugin_H_

#include <Plugins/Compression/Interface/CompressionPlugin.h>
#include <Plugins/Compression/LZss/LZssCompression.h>
#include <Plugins/Compression/LZss/LZss.h>

class LZssPlugin: public CompressionPlugin {
    public:
		bool  Activate();
		void  Release();
		TCHAR *GetGUID();
		TCHAR *GetAuthor();
		TCHAR *GetName();
		unsigned long GetVersion();
		bool IsCertified();
		virtual PluginObject *CreatePluginObject() ;
		virtual void DeletePluginObject(PluginObject *pPlugin) ;
        void SetTraceOutput(TraceOutput *pTraceOutput) ;
		#ifdef EXPORT_SYMBOLS
		_declspec(dllexport) static LZssPlugin *GetInstance();			
		#else
		static LZssPlugin *GetInstance();			
		#endif
	private:
		static LZssPlugin	s_oPlugin;
};

#endif

